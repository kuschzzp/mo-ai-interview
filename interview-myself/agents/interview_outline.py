...
from constants.prompts import INTERVIEW_OUTLINE_PROMPT
from entity.state import State, InterviewOutline
...
def build_prompt(parser):
    template = ChatPromptTemplate(
        messages=[
            ("system", INTERVIEW_OUTLINE_PROMPT),
...
def run(state: State) -> State:
    logger.info("开始执行【面试大纲】生成任务")
    if not state.pdf_text:
        raise Exception("简历文本没有内容！")

    parser = PydanticOutputParser(pydantic_object=InterviewOutline)
    chain = build_prompt(parser) | get_tongyi_langchain_llm(os.getenv("INTERVIEW_OUTLINE_MODEL_NAME"))
    ...
    state.interview_outline = parser.parse(full_response)
    logger.info(state.interview_outline)
    return state
